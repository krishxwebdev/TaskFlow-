...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [req.user.id, title, description || null, priority || 'Medium', dueDate || null, 'Pending']
    );
...
router.get('/summary', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM tasks WHERE user_id = $1', [req.user.id]
    );
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [title, description || null, priority, dueDate || null, req.params.id, req.user.id]
    );
